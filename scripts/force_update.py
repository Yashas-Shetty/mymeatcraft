...
async def force_update_rock8():
    print("Force updating Rock8 Number routing...")
    ...
    # IMPORTANT: Rock8 requires the IDs in the URL for the update endpoint, 
    # but the exact JSON body depends on their backend parsing. We will send 
    # the exact payload with both IDs included to satisfy the Pydantic validator.
    sip_trunk_id = "ST_BDgpwbwNJEJ5"
    dispatch_rule_id = "SDR_SW3mbJAy82ow"
    
    url = f"https://api.rock8.ai/v1/inbound/{sip_trunk_id}/{dispatch_rule_id}"
    print(f"PUT {url}")

    # The exact payload you want
    payload = {
    ...
    async with httpx.AsyncClient() as c:
        response = await c.put(url, headers=headers, json=payload)
        print(f"Status: {response.status_code}")
        print(f"Response: {json.dumps(response.json(), indent=2) if response.status_code == 200 else response.text}")
        
        if response.status_code == 422:
            print("\nTrying again WITHOUT trunk id (Fallback)...")
            del payload["sip_trunk_id"]
            response2 = await c.put(url, headers=headers, json=payload)
            print(f"Fallback Status: {response2.status_code}")
            print(f"Fallback Response: {json.dumps(response2.json(), indent=2) if response2.status_code == 200 else response2.text}")


if __name__ == "__main__":
    asyncio.run(force_update_rock8())
